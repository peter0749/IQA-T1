...
class IQAInference:
    ...
    def _resolve_load_path(self):
        ...
        # 如果是 HF Hub 模型 ID（如 "org/model-name"），直接传给 transformers 下载
        if not os.path.exists(load_path) and "/" in load_path and not load_path.startswith("."):
            return load_path
