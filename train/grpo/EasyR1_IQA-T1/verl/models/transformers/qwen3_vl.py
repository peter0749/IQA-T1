...
def _get_input_embeds(
    ...
    if pixel_values is None and pixel_values_videos is None:
        # This dummy visual call is needed during training to ensure visual module participates in gradient
        # computation. However, it can fail in vLLM inference environment where attention functions are
        # replaced with vLLM-specific implementations that may not be compatible with visual attention.
        try:
            ...
            # Skip dummy visual call in vLLM inference environment
            pass
...
def qwen3_vl_base_forward(
    ...
    **kwargs,
    ...
    if isinstance(position_ids, torch.Tensor) and (position_ids.ndim != 3 or position_ids.size(0) != 4):
        # position_ids is not in expected format (4, batch_size, seq_length)
        # This can happen during vLLM profile/dummy runs where standard position_ids are passed
        # Convert to expected format by expanding 1D/2D position_ids
        if position_ids.ndim == 1:
            # (seq_length,) -> (4, 1, seq_length)
            position_ids = position_ids.unsqueeze(0).unsqueeze(0).expand(4, 1, -1)
        elif position_ids.ndim == 2:
            # (batch_size, seq_length) -> (4, batch_size, seq_length)
            position_ids = position_ids.unsqueeze(0).expand(4, -1, -1)
        else:
            # For other cases, keep original check behavior
    ...
    # Handle both tuple (vLLM) and dataclass (training) outputs
    if isinstance(outputs, tuple):
        hidden_states = outputs[0]
    else:
        hidden_states = outputs.last_hidden_state
    return Qwen3VLModelOutputWithPast(last_hidden_state=hidden_states)
