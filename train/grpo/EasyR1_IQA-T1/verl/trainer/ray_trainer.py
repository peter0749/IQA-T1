...
class RayPPOTrainer:
    ...
    def _make_batch_data(self, metrics: dict[str, Any]) -> DataProto:
        ...
        while True:
            ...
            # 为 agent rollout 保留一份原始多模态数据拷贝，供工具环境使用
            if "multi_modal_data" in new_batch.non_tensor_batch:
                new_batch.non_tensor_batch.setdefault(
                    "origin_multi_modal_data", new_batch.non_tensor_batch["multi_modal_data"]
                )
            ...
            # NOTE: env_name / raw_prompt / origin_multi_modal_data 是 agent 多轮
            # rollout 必需的字段，必须传入 gen_batch，否则 ParallelEnv.reset 中
            # tool_name 为空导致所有工具为 None，多轮生成退化为单轮。
            ...
            if self.config.algorithm.online_filtering:
                ...
                if "env_reward" in new_batch.batch:
                    new_batch.batch["token_level_scores"] = (
                        new_batch.batch["token_level_scores"] + new_batch.batch["env_reward"]
                    )
    ...
    def fit(self):
            ...
            with timer("step", timing_raw):
                ...
                with timer("adv", timing_raw):
                    if "token_level_scores" not in batch.batch:
                        ...
                        # 多轮 agent：将过程奖励 env_reward 与终局 reward 合并
                        if "env_reward" in batch.batch:
                            batch.batch["token_level_scores"] = (
                                batch.batch["token_level_scores"] + batch.batch["env_reward"]
                            )
        ...
        if self.val_reward_fn is not None and self.val_dataloader is not None:
            if (
                val_metrics is None
