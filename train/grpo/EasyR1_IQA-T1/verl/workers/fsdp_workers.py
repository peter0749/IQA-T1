...
class FSDPWorker(Worker):
    ...
    def _process_multi_modal_inputs(self, data: DataProto):
        # Agent rollout 可能提供 multi_modal_inputs（仅含工具调用产生的已处理 Tensor），
        # 而初始 prompt 图像仍在 multi_modal_data（原始格式）中。
        # 我们需要：1) 处理 multi_modal_data -> 得到初始图像的 Tensor；
        #           2) 如果有 agent 提供的工具图像 Tensor，合并到一起。
        agent_mm = data.non_tensor_batch.pop("multi_modal_inputs", None)

        if "multi_modal_data" not in data.non_tensor_batch:
            # 没有原始多模态数据需要处理
            if agent_mm is not None:
                data.non_tensor_batch["multi_modal_inputs"] = agent_mm
            return
        ...
        if agent_mm is not None:
            # 合并：base（初始 prompt 图像，已缓存）+ agent（工具观测图像）
            base_mm = self._cache["multi_modal_inputs"]
            merged = []
            for base_input, agent_input in zip(base_mm, agent_mm):
                if not agent_input:
                    # 该样本没有工具调用图像，直接使用初始 prompt 图像
                    merged.append(base_input)
                elif not base_input:
                    # 该样本没有初始 prompt 图像（不常见），使用工具图像
                    merged.append(agent_input)
                else:
                    # 合并初始 prompt 图像和工具调用图像
                    result = dict(base_input)
                    for key, value in agent_input.items():
                        if key in result:
                            if isinstance(result[key], torch.Tensor) and isinstance(value, torch.Tensor):
                                result[key] = torch.cat([result[key], value], dim=0)
                            elif isinstance(result[key], np.ndarray) and isinstance(value, np.ndarray):
                                result[key] = np.concatenate([result[key], value], axis=0)
                        else:
                            result[key] = value
                    merged.append(result)
            data.non_tensor_batch["multi_modal_inputs"] = np.array(merged, dtype=object)
        else:
